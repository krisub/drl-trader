...
def fetch_and_filter_stream():
    ...
    with open(INTERMEDIATE_FILE, "w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
        writer.writerow(["date", "stock", "title"])
...
def generate_embeddings():
    ...
    df_raw = df_raw.sort_values(["date", "stock"])
    ...
    df_meta = df_raw[["date", "stock"]].reset_index(drop=True)
    df_combined = pd.concat([df_meta, df_emb], axis=1)
    daily_news = df_combined.groupby(["date", "stock"]).mean().reset_index()
